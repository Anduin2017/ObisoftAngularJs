    ...
    .controller("HeaderCtrl", [
        ...
        function ($scope, $location, userInfo, basicApiService) {
            ...
            $scope.logoff = function () {
                basicApiService.logoff().success(function (data) {
                        if (data.Code === 0) {
            ...
            var userInfo = {
                ...
                refreshUsername: function () {
                    basicApiService.signinStatus().success(function (data) {
                        if (data.Code === 0) {
                            basicApiService.userInfo().success(function (data){
                                userInfo.userNick=data.user.nick;
                                userInfo.userIcon=data.user.icon;
                            });
                        }
                        else {
                            $location.path('pages/signin');
